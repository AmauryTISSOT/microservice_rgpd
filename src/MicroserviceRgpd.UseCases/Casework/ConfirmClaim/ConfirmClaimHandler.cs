...
/// et pour la même raison. Ce sont deux écritures et non une transaction, le <c>Ledger</c> étant hors
...
public sealed class ConfirmClaimHandler(IRepository<Case> cases, ILedger ledger, TimeProvider clock)
  : ICommandHandler<ConfirmClaimCommand, Result>
{
  ...
  public async ValueTask<Result> Handle(ConfirmClaimCommand command, CancellationToken cancellationToken)
  {
    ...
    try
    {
      signed = LedgerEntry.ClaimConfirmed(
        command.Case,
        clock.GetUtcNow(),
        command.Right,
