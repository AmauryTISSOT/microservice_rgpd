"""Le moteur témoin : qualification déterministe par lexiques, règles et discriminants.

En marche nominale, cet avis **ne contribue pas au verdict** — il le contrôle. Le jour où le LLM
manque, il devient le verdict de dernier recours. C'est ce double rôle qui justifie qu'il soit
déterministe et sans aucun amont : il ne peut ni tomber en panne de réseau, ni douter de lui-même.

Les règles proviennent des seules recherches sur le vocabulaire CNIL, les articles du RGPD et les
lignes directrices du CEPD — elles n'ont **jamais lu les étiquettes du corpus**. Cette propriété
n'est pas une anecdote de méthode : c'est ce qui autorise à rejouer le corpus en non-régression
sans y mesurer un surapprentissage.

Le moteur raisonne en **slugs français** et n'en sort pas : la traduction vers les noms canoniques
du fil se fait à la frontière du sidecar, jamais ici. Le français est une contrainte locale.

**Ce moteur ne rend aucune confiance.** Aucune constante ne doit lui en donner l'apparence : il
n'a pas d'avis sur sa propre fiabilité, et une confiance constante serait un mensonge typé que
quelqu'un finirait par consommer.
"""
...
#: La version des règles ci-dessous. À incrémenter dès que le comportement du moteur change —
#: le fichier témoin du corpus change alors dans le même commit, et l'écart est délibéré.
ENGINE_VERSION: Final = "1.0.0"

ACCESS: Final = "acces"
RECTIFICATION: Final = "rectification"
ERASURE: Final = "effacement"
RESTRICTION: Final = "limitation"
PORTABILITY: Final = "portabilite"
OBJECTION: Final = "opposition"
OUT_OF_SCOPE: Final = "hors-perimetre"

RIGHTS: Final = (ACCESS, RECTIFICATION, ERASURE, RESTRICTION, PORTABILITY, OBJECTION)
...
def normalise(text: str) -> str:
    ...
    Dé-accentuation : décomposition NFD puis retrait des marques combinantes. Le registre le plus
    dégradé des demandes réelles perd ses accents — les motifs sont donc tous écrits sans accent.
    """
...
# --------------------------------------------------------------------------
#
# ⚠️ Transposition non triviale. La recette anglaise préfixe `NOT_` après tout marqueur de
# négation. Appliquée telle quelle au français, elle **inverse le sens** des demandes les plus
# fréquentes : « ne plus utiliser mes données » et « je ne veux plus recevoir vos offres » *sont*
# l'opposition et la limitation, pas leur négation. Deux corrections :
#
# 1. `plus` est retiré des négateurs — « ne … plus » exprime en français une demande d'arrêt,
#    donc un marqueur positif.
# 2. Les motifs qui intègrent eux-mêmes la négation sont marqués insensibles : sans cela, ils se
...
_STRONG_PUNCTUATION = r"[.;:!?]"


def negation_scopes(text: str) -> list[tuple[int, int]]:
    """Intervalles [début, fin) sous portée d'une négation."""
    scopes: list[tuple[int, int]] = []
    for match in re.finditer(_NEGATIONS, text):
        end = re.search(_STRONG_PUNCTUATION, text[match.end():])
        scopes.append((match.start(), match.end() + (end.start() if end else len(text))))
    return scopes


def under_negation(text: str, position: int) -> bool:
    return any(start <= position < end for start, end in negation_scopes(text))


# --------------------------------------------------------------------------
# Lexiques par droit
#
# Chaque entrée : (motif, poids, sensible_à_la_négation). Le poids reflète la force du marqueur —
# une citation d'article vaut plus qu'un verbe polysémique. Le troisième champ vaut `False` pour
# les motifs qui intègrent déjà une négation (cf. le bloc « Négation » ci-dessus).
#
# `DATA` couvre les graphies dégradées : après dé-accentuation, « données », « donnée », « donné »
# et « donnés » donnent respectivement donnees, donnee, donne et donnes.
# --------------------------------------------------------------------------

DATA = r"donn(?:e|ee)s?"
INFO = rf"(?:{DATA}|informations?|infos?|renseignements?)"

LEXICONS: dict[str, list[tuple[str, int, bool]]] = {
    ACCESS: [
        (r"article\s*1?5\b(?!\d)", 5, True),
        (r"\bdroit d'?\s*acces\b", 5, True),
        (rf"\bacceder a mes {DATA}\b", 4, True),
        (r"\bcopie (?:de|des|integrale|complete|en langage clair)", 3, True),
        (rf"\bquelle?s? {INFO}\b.{{0,40}}\b(?:detenez|avez|possedez|gardez|conservez|figurent|utilisez)", 4, True),
        (r"\bsavoir (?:si|quelles|ce que|cke)\b", 3, True),
        (r"\bsuis[- ]je fiche", 4, True),
        (r"\bje suis dans vos fichiers\b", 4, True),
        (rf"\bconnaitre (?:les|mes|quelles?|la liste) (?:{INFO}|comptes)\b", 3, True),
        (r"\bcommuniquez[- ]moi mon dossier\b", 4, True),
        (rf"\bm'?indiquer si des {DATA}\b", 4, True),
        (rf"\borigine (?:de ces|des|de mes|de vos) {DATA}\b|\bl'?origine (?:de ces|des) {DATA}\b", 4, True),
        (r"\bd'?ou (?:viennent|provienn?ent)\b", 3, True),
        (r"\bduree de conservation\b", 3, True),
        (rf"\b(?:finalites|destinataires)\b.{{0,30}}\b(?:traitement|{DATA})\b", 3, True),
        ...
        (rf"\bconsulter (?:mon dossier|mes {DATA})\b|\bvoir mon dossier\b|\bvoire mon dossier\b", 4, True),
        (rf"\bliste (?:de|des) (?:mes )?{INFO}\b", 3, True),
    ...
    RECTIFICATION: [
        ...
        # « mise à jour » nominal est écarté : il désigne le plus souvent la mise à jour d'une
        # politique ou d'un service, pas une demande de l'art. 16.
    ...
    ERASURE: [
        ...
        (rf"\benlev(?:er|ez)\b.{{0,30}}\b(?:{DATA}|informations|fichier|profil|photo)\b", 3, True),
    ...
    PORTABILITY: [
        ...
        (rf"\brecuperer (?:mes|toutes mes|les) (?:{DATA}|photos|messages)\b", 3, True),
    ...
    OBJECTION: [
        ...
        (rf"\bne (?:soient|soit|servent) plus\b|\bque mes {DATA} (?:ne )?servent\b|\bque mes achats servent\b", 3, False),
...
# ⚠️ `\bavoir\b` a été retiré : captait « je ne veux plus rien avoir à faire avec vous », qui est
# une demande d'effacement. Un motif de litige commercial doit porter sur un terme non polysémique.
NOT_A_REQUEST = [
...
OUT_OF_SCOPE_PATTERNS = [
    re.compile(pattern)
    for pattern in OUT_OF_TAXONOMY + ELSEWHERE + NOT_A_REQUEST + QUESTION_ABOUT_A_RIGHT
]

# Marqueurs d'exercice effectif d'un droit : neutralisent les indices « pas une demande » quand le
# texte formule aussi une demande RGPD explicite.
...
# --------------------------------------------------------------------------
# Discriminants des paires confondues
# --------------------------------------------------------------------------

REPLACEMENT_VALUE = re.compile(
...
def qualify(text: str) -> list[str]:
    """Rend la liste **non vide** des slugs qualifiés pour ce texte, hors périmètre exclusif.
    ...
    retained = [right for right in RIGHTS if scores[right] >= THRESHOLD]
    ...
    # --- Repli sous le seuil ------------------------------------------------
    # Le CEPD demande d'interpréter largement une demande ambiguë plutôt que de la rejeter
    # (*Guidelines 01/2022* § 50). Un indice faible mais présent, sur un texte qui parle bien de
    # données personnelles, vaut mieux qu'un rejet.
    if best >= FALLBACK_THRESHOLD and exercise:
        return [right for right in RIGHTS if scores[right] == best]
