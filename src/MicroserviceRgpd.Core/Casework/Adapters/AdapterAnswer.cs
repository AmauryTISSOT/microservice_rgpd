...
/// <summary>
/// Ce qu'un appel d'<c>Adapter</c> rapporte : un <see cref="AdapterVerdict"/>, et ce que ce verdict
/// porte avec lui — le corps servi, ou l'<b>échéance déclarée</b> d'un différé.
/// </summary>
...
public sealed record AdapterAnswer<TServed>
  where TServed : class
{
  private AdapterAnswer(AdapterVerdict verdict, TServed? served, DateTimeOffset? declaredDeadline)
  {
    Verdict = verdict;
    Served = served;
  ...
  public AdapterVerdict Verdict { get; }

  /// <summary>
  /// Ce qui a été servi, ou <c>null</c> dès que le verdict n'est pas <see cref="AdapterVerdict.Served"/>.
  /// Le <c>null</c> ne remplace ici aucun corps : il accompagne un verdict qui dit déjà qu'aucun
  /// n'a été rendu.
  /// </summary>
  public TServed? Served { get; }

  /// <summary>
  /// L'échéance que l'<c>Adapter</c> a <b>déclarée</b> en différant, ou <c>null</c> pour tout autre
  /// verdict. Elle est déclarée, jamais négociée : le service la relit et repassera après elle,
  ...
  public static AdapterAnswer<TServed> Serving(TServed served)
  {
    ...
    return new AdapterAnswer<TServed>(AdapterVerdict.Served, served, declaredDeadline: null);
  }
  ...
  /// Le verdict donné n'est pas un refus. Servir et différer ont chacun leur fabrique, parce que
  /// chacun porte quelque chose que celle-ci n'a pas.
  /// </exception>
  public static AdapterAnswer<TServed> Refusing(AdapterVerdict refusal)
  {
    ...
    if (!refusal.IsRefusal)
    {
      throw new ArgumentException(
        $"« {refusal.Name} » n'est pas un refus : servir rend un corps, différer rend une échéance, "
        + "et chacun a sa fabrique.",
        nameof(refusal));
    }

    return new AdapterAnswer<TServed>(refusal, served: null, declaredDeadline: null);
  }
}
