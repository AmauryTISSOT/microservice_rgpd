...
/// <summary>
/// L'écran d'<b>une table d'un rapport de détection archivé</b> : toutes ses colonnes, dans l'ordre
/// du relevé, et ce qu'un humain en avait tranché — <b>sans un seul bouton</b>.
/// </summary>
/// <remarks>
/// <para>
/// ⚠️ <b>Aucun filtre, ici non plus.</b> Un archivé se lit <b>en entier</b> : les
/// <c>Unflagged</c> <b>sont</b> l'écran, et un rapport de détection ancien dont on ne rendrait que
/// les signalées serait un rapport de détection dont personne ne peut plus vérifier ce que la
/// détection n'avait pas vu.
/// </para>
/// <para>
...
/// </para>
/// <para>
/// <b>Le schéma, la table et le rapport de détection passent en paramètres de requête, jamais dans
/// le chemin</b> :
...
{
  /// <summary>Le rapport de détection archivé dont on ouvre une table.</summary>
  ...
  public async Task<IActionResult> OnGetAsync(CancellationToken cancellationToken)
  {
    ...
    // Une adresse sans ses deux membres ne désigne aucune table : on renvoie au rapport de
    // détection archivé, qui les nomme toutes. Forger une identité sur un membre vide aurait levé
    // au fond d'un domaine dont la règle est qu'un triplet mal formé est une programmation fautive,
    // jamais une saisie.
    ...
    // Le rapport de détection a été supprimé, il est redevenu le courant, ou il ne portait pas
    // cette table. Le sommaire de l'archivé la nommerait s'il l'avait ; et s'il n'existe plus,
    // c'est lui qui renvoie à l'historique.
