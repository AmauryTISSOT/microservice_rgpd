...
Vingt-neuf ADR de système sont en vigueur. Un ADR supplanté n'est jamais édité : la supplantation est
écrite dans l'ADR qui supplante, toujours sur un point nommé. Six ADR portent en outre, en fin de
...
ADR-0026 et 0027, que l'ADR-0028 a supplantés, et les ADR-0021 et 0023, que l'ADR-0029 a supplantés
— la date limite de réponse cesse d'y être immuable, et une modification n'y efface plus une
prolongation. Les autres ADR supplantés n'en portent pas :
...
| [0029](./docs/adr/0029-prolonger-une-demande-est-un-geste-une-seule-fois-de-deux-mois.md) | Prolonger une demande est un `Gesture` : une seule fois, de deux mois (art. 12 §3), sur un motif fermé à deux valeurs — complexité, nombre de demandes — **et** une justification écrite obligatoire, premier texte libre saisi par l'`Operator`. Prolonger n'est possible que tant que la date limite n'est pas passée ; la prolongation tient sur l'agrégat en quatre valeurs, la date limite initiale est enregistrée et non dérivée, `Modify` recalcule les deux dates ensemble et corriger la date de réception est toujours accepté. Le service n'enregistre pas qui a prolongé, et l'information de la personne concernée reste hors du service. Aucun code : la décision précède l'implémentation. | 0021 : « elle est fixée par `Receive` et enregistrée ; elle ne se recalcule pas », et « la prolongation du délai (art. 12.3) » parmi ce qu'il n'ouvrait pas. 0023 : « la date limite de réponse est refaite par toute modification effective », et la même clause de fermeture. |
