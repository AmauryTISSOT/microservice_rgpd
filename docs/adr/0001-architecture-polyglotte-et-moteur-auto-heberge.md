...
**L'auto-hébergement plutôt qu'une API tierce.** Tout appel à un tiers serait une sous-traitance au sens de l'art. 28 du RGPD, sur un texte non caviardé. L'auto-hébergement supprime le sous-traitant, l'engagement contractuel et la question de la localisation **par construction** — il n'y a rien à écrire noir sur blanc. Un microservice dont l'unique métier est la conformité RGPD ne peut pas être celui qui expédie des demandes d'exercice de droits chez un tiers sans y avoir réfléchi.

**L'hybride plutôt qu'un moteur seul.** Ce qu'il apporte et qu'aucun moteur seul n'apporte : **l'alarme cesse de reposer sur l'auto-évaluation du modèle**. Les petits modèles sont réputés sur-confiants ; le lexique déclenche l'alarme **de l'extérieur**, sans rien demander au LLM. C'est un signal **indépendant**, structurellement plus solide qu'une introspection.
...
**Le sidecar Python — le point le plus discutable, et il faut être franc.** **Rien ne l'impose techniquement** : le lexique n'importe que `re` et `unicodedata`, que .NET fait aussi bien. La frontière est posée pour un **motif unique et explicite : une évolution vers l'apprentissage automatique est anticipée** (plongements, classifieur entraîné, éventuel *fine-tuning*). Mieux vaut la poser maintenant, tant qu'elle est bon marché, que la découvrir plus tard. **Si ce motif tombait, cette décision serait à revoir.** Bénéfice immédiat non trivial : le harnais d'évaluation qui a produit les chiffres du prototypage évalue désormais le **code de production**, et non une maquette.
...
- **La taxonomie existe des deux côtés d'une frontière.** Traité par un fichier JSON de projection lu par Python et gardé par un test unitaire .NET, `Core` restant la source de vérité.
