...
- **Renverse deux clauses de glossaire**, pas un ADR — comme l'ADR-0012 : la doctrine « rien n'est
  obligatoire au dépôt », et le défaut `J+9`, écrits dans le glossaire de `Casework`. Ce glossaire
  a été supprimé par l'[ADR-0017](./0017-casework-est-retire-requests-le-remplace.md) ; les deux
  clauses ne sont pas tombées avec lui, elles sont renversées ici, et le glossaire de `Requests`
  écrit déjà l'inverse.
...
faible doit rester enregistrable et **visible comme tel**. » Le glossaire de `Casework` en portait
les deux formes :
...
`DataSubjectRequest.Receive`, qu'appelle le gestionnaire de la commande d'enregistrement. Elle ne
s'arrête pas à la première faute : elle rend **toutes** les raisons de refuser la saisie, chacune
rattachée à un champ. Le handler de la page, `POST /demandes?handler=Create`, répond 201, ou 400
`ValidationProblem` indexé par champ, et dans ce cas rien n'est enregistré. Une requête envoyée directement au serveur, sans passer par
l'écran, ne peut pas enregistrer une demande invalide.
...
endroit côté serveur, `ParisCalendar`, lu par le gestionnaire de la commande et par la page ; le
...
- **Accepter « hors périmètre », ou un « droit inconnu ».** Le droit invoqué est ce que la demande
  ...
  mais désactivée.
- **Valider dans le navigateur seulement.** Une requête envoyée directement au serveur aurait
...
JavaScript ; la divergence est tenue par les mêmes mesures des deux côtés, par le serveur qui fait
foi, et par les tests aux trois coutures — domaine, HTTP, navigateur (ADR-0018, ADR-0020). Un plafond
