...
public class RequestSearch(BrowserHarness harness)
{
  ...
  /// <summary>
  /// ⚠️ <b>Une suppression pendant une recherche la tient à jour</b> : la seule demande trouvée
  /// supprimée, « Aucune demande ne correspond à votre recherche » paraît — et non un tableau blanc,
  /// sans un mot, au-dessus de lignes que la recherche cache.
  /// </summary>
  [Fact]
  public async Task SaysNothingMatchesOnceTheOnlyMatchIsDeleted()
  {
    var marker = Marker();
    await harness.RecordRequestAsync(email: $"{marker}@example.org");
    var other = Marker();
    ...
    await DeleteAsync(page, marker);

    await Expect(Row(page, marker)).ToHaveCountAsync(0);
    await Expect(NoMatchMessage(page)).ToBeVisibleAsync();
    await Expect(Row(page, other)).ToBeHiddenAsync();
    await Expect(page.GetByText(NoRequestYet, new() { Exact = true })).ToBeHiddenAsync();
  }
  ...
  /// <summary>
  /// ⚠️ <b>La dernière demande supprimée pendant une recherche, le tableau vide le dit seul</b> :
  /// « Aucune demande pour le moment », et non les deux messages à la fois.
  /// </summary>
  [Fact]
  public async Task SaysOnlyThereIsNoRequestYetOnceTheLastRequestIsDeletedDuringASearch()
  {
    await harness.DeleteAllRequestsAsync();
    var marker = Marker();
    await harness.RecordRequestAsync(email: $"{marker}@example.org");
  ...
  /// <summary>Supprime, par sa poubelle et sa confirmation, la demande affichée qui porte <paramref name="text"/>.</summary>
  private static async Task DeleteAsync(IPage page, string text)
  {
    await Row(page, text).GetByRole(AriaRole.Button, new() { Name = "Supprimer la demande", Exact = true }).ClickAsync();
    await page.GetByRole(AriaRole.Alertdialog, new() { Name = "Supprimer la demande", Exact = true })
      .GetByRole(AriaRole.Button, new() { Name = "Supprimer définitivement", Exact = true })
      .ClickAsync();
  }
