...
| 2026-08-09 | openemr (25 désaccords sur 29 de Pile B) | `ODSPH`, `CC3`, `TIMING2`, `OSMPDD`, `LENS_TREATMENTS`, `codetext`… familles `form_eye_*`, `history_data` | Une colonne dont le nom ne rend aucun mot lisible, dans une table dont le nom nomme un domaine sans ambiguïté, hérite-t-elle de ce domaine ? Le § 3.3 ne tranche que les **champs libres** ; hors d'eux, **aucune règle n'existait**. La passe machine a hérité (`HealthData`), la passe humaine a refusé (`Unflagged`). ⚠️ **Question posée après avoir vu le désaccord** — l'ordre habituel du journal n'est pas tenu, et c'est dit plutôt que masqué. | **Oui, et en faveur de la machine.** Une colonne dont le nom ne dit rien prend la catégorie du domaine que le nom de sa table nomme. **Borne** : l'héritage porte le domaine de la table, **jamais** la lecture de la colonne — le motif cite la table et ne dit rien de la colonne. Priment : § 3.1, 3.2, 3.3, 3.4. **Fondement vérifié et non supposé** : aucune des 17 tables `form_eye_*` ne porte de `commentaire_table` (0 sur 510 colonnes), donc le nom de table est le seul signal sur `ODSPH`, et il est dans le pivot. | **oui, § 3.10** (nouveau) |
| 2026-08-09 | openemr (3 col.) | `billing.encounter`, `form_group_attendance.encounter_id`, `form_eye_mag_wearing.ENCOUNTER` | ⚠️ **Incohérence trouvée par la passe de vérification du § 3.10, et qui n'est pas du § 3.10** : la même famille porte deux étiquettes — `HealthData` sur les deux premières, `Unflagged` sur la troisième. `encounter` **dit** quelque chose, donc la condition 1 du § 3.10 n'est pas remplie ; la colonne relève du § 3.2, dont les deux branches — personne et nomenclature — ne couvrent **ni l'une ni l'autre** une clé étrangère vers une table d'**objet**. C'est le trou interne au § 3.2 que le classement range en Pile C. | **Alignement sur la lecture majoritaire, sans nouveau §.** `form_eye_mag_wearing.ENCOUNTER` passe à `HealthData` : elle désigne la même rencontre de soins que `form_group_attendance.encounter_id`, déjà étiquetée ainsi. ⚠️ **C'est un second arbitrage post-hoc en faveur de la machine**, plus petit que le § 3.10 mais de même nature, et il se déclare comme tel. Il **ne ferme pas** le trou du § 3.2 sur les clés étrangères vers un objet, qui reste ouvert et sans règle. **1 colonne repassée.** | **non** — un seul cas aligné, la famille reste sans règle |
